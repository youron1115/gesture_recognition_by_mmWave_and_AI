...
def fit_model(train_data, train_labels, valid_data, valid_labels, model_path):   
    ...
    model.save(os.path.join(model_path, 'gesture_model_RDI_data_no_normalize.h5'))
    print("\nsave complete")
...
train_data = np.load(os.path.join(processed_data_path, 'train_no_normalize.npz'))
train_labels = train_data['labels']
train_data = train_data['data']

valid_data = np.load(os.path.join(processed_data_path, 'val_no_normalize.npz'))
