...
def save_data(train_data, train_labels, val_data, val_labels, test_data, test_labels, output_dir):
    ...
    np.savez_compressed(os.path.join(output_dir, 'train_no_normalize.npz'), data=train_data, labels=train_labels)
    np.savez_compressed(os.path.join(output_dir, 'val_no_normalize.npz'), data=val_data, labels=val_labels)
    np.savez_compressed(os.path.join(output_dir, 'test_no_normalize.npz'), data=test_data, labels=test_labels)
