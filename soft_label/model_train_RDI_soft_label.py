...
def fit_model(train_data, train_labels, valid_data, valid_labels, model_path):   
    ...
    model.fit(train_data, train_labels, validation_data=(valid_data, valid_labels), epochs=440, batch_size=int(train_data.shape[0]/5.0), shuffle=True)
    print("\nTraining complete")
...
train_data = np.load(os.path.join(processed_data_path, 'train_soft_label.npz'))
train_labels = train_data['labels']
train_data = train_data['data']

valid_data = np.load(os.path.join(processed_data_path, 'val_soft_label.npz'))
