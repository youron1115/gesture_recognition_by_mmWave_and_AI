...
train_or_test_data='test_soft_label.npz'
...
output=pd.DataFrame(columns=['Predicted','Actual','Right or False','Predicted Probability Value',])#'True Probability Value'

# Convert predicted values to predict labels
pred_labels = []
for i in range(len(predicted_value)):
    pred_labels.append(np.argmax(predicted_value[i]))
output['Predicted'] = np.array(pred_labels)
print("Predicted labels")

# Convert test value to true labels
true_labels=[]
for i in range(len(test_labels)):
    true_labels.append(np.argmax(test_labels[i]))
output['Actual'] = np.array(true_labels)
print("Actual labels")

#compare predict labels and true labels
right_or_false=[]
...
output['True Probability Value'] = true_labels
print("True Probability Value")
"""

output_path=os.path.join(current_path, "test_output/predicted_labels_RDI_soft_label.csv")
